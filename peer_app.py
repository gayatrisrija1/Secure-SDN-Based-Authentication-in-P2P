#!/usr/bin/env python3
"""
Peer Backend - Anonymous P2P Communication
Handles secure peer discovery, authentication, and encrypted messaging
"""
...
class AnonymousPeer:
    def __init__(self, port=8080):
        ...
        app.logger.info(f"Peer initialized with pseudo-ID: {self.pseudo_id}")
    ...
    def register_with_controller(self):
        ...
        try:
            ...
            if response.status_code == 200:
                app.logger.info(f"Successfully registered with controller")
                # Start session reporting
                self._start_session_reporting()
                return True
            else:
                app.logger.warning(f"Controller registration failed: {response.status_code}")
                return False
                
        except Exception as e:
            app.logger.warning(f"Could not register with controller: {e}")
            return False
        ...
        def report_sessions():
            while self.running:
                try:
                    ...
                    app.logger.info(f"Reporting session: {session_data}")
                    
                    response = requests.post('http://localhost:5000/api/session/report', 
                                json=session_data, timeout=3)
                    
                    if response.status_code == 200:
                        app.logger.info(f"Session report successful: {response.json()}")
                    else:
                        app.logger.error(f"Session report failed: {response.status_code}")
                    
                except Exception as e:
                    app.logger.error(f"Session reporting error: {e}")
    ...
    def start_discovery(self):
        ...
        try:
            ...
            self.discovery_socket.bind(('', 9999))
            self.running = True
            ...
            app.logger.info("Peer discovery started on port 9999")
            return True
            
        except Exception as e:
            app.logger.error(f"Failed to start discovery: {e}")
            return False
            ...
            try:
                ...
                if message.get('type') == 'PEER_DISCOVERY':
                    ...
                    if peer_info['pseudo_id'] != self.pseudo_id:
                        ...
                        app.logger.info(f"Discovered peer: {peer_info['pseudo_id']} at {addr[0]}")
                        
            except Exception as e:
                if self.running:
                    app.logger.error(f"Discovery listener error: {e}")
    ...
    def authenticate_with_peer(self, peer_id):
        ...
        try:
            ...
            if response and response.get('status') == 'pending':
                ...
                app.logger.info(f"Authentication request sent, awaiting approval. Request ID: {request_id}")
                ...
                if response.get('response_hash') == expected_response:
                    ...
                    app.logger.info(f"Authentication successful with peer {peer_id}")
    ...
    def _send_peer_message(self, ip_address, port, message):
        ...
        try:
            ...
            app.logger.error(f"Peer message error: {e}")
            return None

    def send_encrypted_message(self, message_text):
        """Send encrypted message to active session peer"""
        if not self.active_session or not self.session_key:
            return False, "No active session"
            
        try:
            # Encrypt message
            encrypted_message = self._encrypt_message(message_text, self.session_key)
            if not encrypted_message:
                return False, "Encryption failed"
                
            message = {
                'type': 'ENCRYPTED_MESSAGE',
                'sender_pseudo_id': self.pseudo_id,
            ...
            # Get peer info
            peer_info = self.discovered_peers.get(self.active_session['peer_id'])
            if not peer_info:
                return False, "Peer not available"
                
            # Send message
            response = self._send_peer_message(peer_info['ip_address'], peer_info['port'], message)
            ...
            if response and response.get('status') == 'received':
                ...
                app.logger.info("Encrypted message sent successfully")
    ...
    def terminate_session(self):
        ...
        if self.active_session:
            app.logger.info(f"Terminating session with {self.active_session['peer_id']}")
    ...
    def handle_security_alert(self, alert_type, message):
        """Handle security alerts from SDN controller"""
        alert = {
            'timestamp': time.time(),
            'type': alert_type,
            'message': message,
        ...
        if alert_type == 'SESSION_TERMINATION':
            self.terminate_session()
        elif alert_type == 'KEY_DESTRUCTION':
            self.session_key = None
            if self.active_session:
                self.active_session['authenticated'] = False
        elif alert_type == 'PEER_ISOLATION':
            self.terminate_session()
            self.running = False
            
        app.logger.warning(f"Security alert: {alert_type} - {message}")
...
def get_peer_status():
    ...
    # Always check current session state first
    active_session_status = peer_instance.active_session is not None
    
    status = {
        'pseudo_id': peer_instance.pseudo_id,
        'running': peer_instance.running,
        'active_session': active_session_status,
        'discovered_peers': len(peer_instance.discovered_peers),
        'message_count': len(peer_instance.message_history)
    }
    
    # Debug logging
    app.logger.info(f"Status check - active_session: {active_session_status}, session_obj: {peer_instance.active_session}")
    
    # Check if we have a pending outgoing request
    if hasattr(peer_instance, 'pending_auth') and peer_instance.pending_auth:
        # Check if the request was accepted by polling the target
        try:
            peer_info = peer_instance.pending_auth['peer_info']
            check_url = f"http://{peer_info['ip_address']}:{peer_info['port']}/api/connection/check/{peer_instance.pending_auth['request_id']}"
            ...
            if response.status_code == 200:
                ...
                if data.get('status') == 'accepted':
                    ...
                    expected_response = hashlib.sha256(f"{data.get('pseudo_id')}{nonce}{timestamp}".encode()).hexdigest()
                    
                    if data.get('response_hash') == expected_response:
                        # Create session
                        ...
                        peer_instance.active_session = {
                            ...
                            'peer_pseudo_id': data.get('pseudo_id'),
                            'start_time': time.time(),
                            'authenticated': True
                        }
                        ...
                        app.logger.info("Authentication completed successfully")
                        
                elif data.get('status') == 'rejected':
                    # Clear pending auth
                    peer_instance.pending_auth = None
                    app.logger.info("Authentication request was rejected")
                    
        except Exception as e:
            # If request is too old (>30 seconds), clear it
            if time.time() - peer_instance.pending_auth['sent_at'] > 30:
                peer_instance.pending_auth = None
                app.logger.info("Authentication request timed out")
...
def get_security_alerts():
    """Get security alerts"""
    app.logger.info(f"Security alerts requested - count: {len(peer_instance.security_alerts)}")
    return jsonify({
...
def handle_auth_request():
    ...
    try:
        ...
        try:
            ...
            app.logger.debug(f"✓ Reported auth attempt to controller (requester: {requester_id[:8]}...)")
        except Exception as e:
            app.logger.error(f"✗ Failed to report to controller: {e}")
        ...
        if hasattr(peer_instance, 'blocked_peers') and requester_id in peer_instance.blocked_peers:
            app.logger.warning(f"🚫 BLOCKED connection request from {requester_id[:8]}... (peer is isolated/blocked)")
            return jsonify({
                'status': 'blocked',
        ...
        if challenge == expected_challenge:
            ...
            app.logger.info(f"Connection request received from {requester_id}")
    ...
    try:
        ...
        if (hasattr(peer_instance, 'pending_auth') and 
            ...
            if response_hash == expected_response:
                # Use the shared session key from the accepter
                ...
                app.logger.info(f"Connection accepted by {pseudo_id} - shared key received")
        ...
        app.logger.error(f"Error handling connection acceptance: {e}")
        return jsonify({'status': 'error', 'message': str(e)}), 500

@app.route('/api/connection/check/<request_id>', methods=['GET'])
def check_request_status(request_id):
    """Check if a connection request was accepted/rejected"""
    # Check completed requests
    completed_requests = getattr(peer_instance, 'completed_requests', {})
    
    if request_id in completed_requests:
        return jsonify(completed_requests[request_id])
    
    # If request is still pending
    if request_id in peer_instance.pending_requests:
        return jsonify({
            'status': 'pending',
            'message': 'Request still awaiting approval'
        })
...
def respond_to_request():
    ...
    try:
        ...
        if action == 'accept':
            ...
            # Notify the requester directly with the shared key
            try:
                ...
                app.logger.error(f"Failed to notify requester: {e}")
            
            # Remove from pending
            del peer_instance.pending_requests[request_id]
            
            app.logger.info(f"Connection accepted from {req_data['requester_id']}")
            ...
            app.logger.info(f"Connection rejected from {req_data['requester_id']}")
...
def receive_peer_message():
    ...
    try:
        ...
        if data.get('type') == 'FLOOD_PACKET':
            sender_id = data.get('pseudo_id')
            
            # Report flooding attempt to controller
            try:
                monitor_data = {
...
def send_peer_message():
    ...
    try:
        ...
        app.logger.info(f"Sending message to peer {peer_id}: {message_text[:50]}...")
        ...
        if peer_info:
            try:
                ...
                app.logger.info(f"Send response: {response.status_code} - {response.text}")
            except Exception as e:
                app.logger.error(f"Failed to send to peer: {e}")
                return jsonify({'status': 'error', 'message': f'Failed to deliver message: {e}'}), 500
...
def get_peer_messages():
    ...
    app.logger.info(f"Messages requested - count: {len(peer_instance.message_history)}")
    return jsonify({
        'messages': peer_instance.message_history,
        'session_active': peer_instance.active_session is not None,
        'session_info': {
            'peer_id': peer_instance.active_session['peer_id'] if peer_instance.active_session else None,
            'has_key': peer_instance.session_key is not None
        }
    })
...
def receive_security_alert():
    ...
    try:
        ...
        app.logger.warning(f"SECURITY ALERT RECEIVED: {security_alert['type']} - {security_alert['severity']} - {security_alert['message']}")
        ...
        app.logger.info(f"Processing progressive action notification: {notification_type} for attacker {attacker_id[:8] if attacker_id else 'unknown'}...")
        ...
        if notification_type == 'SESSION_TERMINATED':
            ...
            if peer_instance.active_session:
                if peer_instance.active_session.get('peer_id') == attacker_id or \
                   peer_instance.active_session.get('peer_pseudo_id') == attacker_id:
                    ...
                    app.logger.warning(f"🔒 SESSION TERMINATED by controller (attacker: {attacker_id[:8]}...)")
                else:
                    app.logger.info(f"SESSION_TERMINATED notification received but not connected to attacker (connected to {peer_instance.active_session.get('peer_id', 'unknown')[:8]}...)")
            else:
                app.logger.info(f"SESSION_TERMINATED notification received but no active session")
            ...
            for req_id in requests_to_remove:
                ...
                app.logger.info(f"Removed pending connection request from blocked attacker {attacker_id[:8]}...")

            app.logger.info(f"Blocked future connections from {attacker_id[:8]}...")
            ...
            if peer_instance.active_session:
                if peer_instance.active_session.get('peer_id') == attacker_id or \
                   peer_instance.active_session.get('peer_pseudo_id') == attacker_id:
                    ...
                    peer_instance.session_key = None
                    peer_instance.message_history = []
                    app.logger.error(f"🔐 KEYS DESTROYED by controller (attacker: {attacker_id[:8]}...)")
                else:
                    app.logger.info(f"KEYS_DESTROYED notification received but not connected to attacker (connected to {peer_instance.active_session.get('peer_id', 'unknown')[:8]}...)")
            else:
                app.logger.info(f"KEYS_DESTROYED notification received but no active session")
            ...
            if peer_instance.active_session:
                if peer_instance.active_session.get('peer_id') == attacker_id or \
                   peer_instance.active_session.get('peer_pseudo_id') == attacker_id:
                    ...
                    peer_instance.session_key = None
                    peer_instance.message_history = []
                    app.logger.critical(f"🚫 SESSION with attacker TERMINATED (attacker: {attacker_id[:8]}...)")
                else:
                    app.logger.info(f"PEER_ISOLATED notification received but not connected to attacker (connected to {peer_instance.active_session.get('peer_id', 'unknown')[:8]}...)")
            ...
            for pid in peers_to_remove:
                ...
                app.logger.info(f"Removed attacker from discovered peers list")
            ...
            for req_id in requests_to_remove:
                del peer_instance.pending_requests[req_id]
                app.logger.info(f"Removed pending connection request from isolated attacker {attacker_id[:8]}...")

            app.logger.critical(f"🚫 ATTACKER ISOLATED AND BLOCKED (attacker: {attacker_id[:8]}...)")

        return jsonify({'status': 'success', 'alert_received': True, 'action_taken': True})

    except Exception as e:
        app.logger.error(f"Error receiving security alert: {e}")
        return jsonify({'status': 'error', 'message': str(e)}), 500
@app.route('/api/peer/terminate', methods=['POST'])
def terminate_peer_session():
